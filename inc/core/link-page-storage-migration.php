...
function extrachill_analytics_link_page_migration_table_ready( $table, $columns, $indexes ) {
	...
	foreach ( $column_rows ? $column_rows : array() as $row ) {
		$actual_columns[ $row['Field'] ] = strtolower( $row['Type'] ) . '|' . $row['Null'] . '|' . (string) $row['Default'] . '|' . strtolower( $row['Extra'] );
	}
...
function extrachill_analytics_link_page_migration_plan( $context ) {
	...
	try {
		...
		$ready        = extrachill_analytics_link_page_migration_table_ready(
			$views_table,
			array(
				'view_id'      => 'bigint(20) unsigned|NO||auto_increment',
				'link_page_id' => 'bigint(20) unsigned|NO||',
				'stat_date'    => 'date|NO||',
				'view_count'   => 'bigint(20) unsigned|NO|0|',
			),
			array(
				'PRIMARY'           => array(
					'unique'  => true,
		...
		$ready = extrachill_analytics_link_page_migration_table_ready(
			$clicks_table,
			array(
				'click_id'     => 'bigint(20) unsigned|NO||auto_increment',
				'link_page_id' => 'bigint(20) unsigned|NO||',
				'stat_date'    => 'date|NO||',
				'link_url'     => 'varchar(2083)|NO||',
				'link_text'    => 'varchar(255)|NO||',
				'click_count'  => 'bigint(20) unsigned|NO|0|',
			),
			array(
				'PRIMARY'                 => array(
					'unique'  => true,
		...
		foreach ( $source['clicks'] as $row ) {
			$collision = $wpdb->get_var( $wpdb->prepare( "SELECT click_id FROM `{$clicks_table}` WHERE click_id = %d OR (link_page_id = %d AND stat_date = %s AND link_url = %s AND link_text = %s) LIMIT 1", $row['click_id'], $row['link_page_id'], $row['stat_date'], $row['link_url'], $row['link_text'] ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Owned table preflight.
			if ( $collision ) {
	...
	if ( function_exists( 'extrachill_analytics_events_table' ) ) {
		$needles = array_map(
			...
			$context['link_page_ids']
		);
		foreach ( $needles as $needle ) {
			$rows       = $wpdb->get_results( $wpdb->prepare( 'SELECT id, event_type, blog_id, source_url FROM `' . extrachill_analytics_events_table() . '` WHERE blog_id = %d AND event_data LIKE %s ORDER BY id', $source_blog_id, $needle ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Historical identities are reported, never rewritten.
			$historical = array_merge( $historical, $rows ? $rows : array() );
		}
	}
	$data                = array(
		'rows'                      => $source,
...
function extrachill_analytics_link_page_migration_rollback( $context ) {
	...
	try {
		...
		foreach ( array_reverse( $context['journal_entries'] ?? array() ) as $entry ) {
			if ( 'participant' !== ( $entry['type'] ?? '' ) || 'analytics' !== ( $entry['participant'] ?? '' ) || ! empty( $entry['rolled_back'] ) ) {
				continue; }
