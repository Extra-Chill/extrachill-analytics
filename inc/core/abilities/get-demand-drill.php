...
function extrachill_analytics_register_demand_drill_ability() {
	wp_register_ability(
		...
		array(
			...
			'description'         => __( 'Attribute a surface\'s declining demand slope to the specific pages and queries dragging it. Pulls Google Search Console per-page and per-query stats for the current window and the immediately-preceding equal window, joins them, and returns the top decliners and risers ranked by net click change, each with current AND prior average position so a rank-loss is visible. Reuses the datamachine/google-search-console primitive (no parallel GSC client). Unmeasurable dimensions return a not_instrumented coverage marker, never a zero.', 'extrachill-analytics' ),
...
function extrachill_analytics_ability_get_demand_drill( $input ) {
	...
	return array(
		...
		'note'           => 'Contributors to a surface demand slope, ranked by NET CLICK CHANGE (current-window clicks minus prior equal-window clicks) from Google Search Console page_stats/query_stats. Decliners are the pages/queries that lost the most clicks; risers gained the most. Each carries current AND prior average position: a decliner whose position rose (worsened) lost rank (rank-recovery lever); a decliner whose position held but clicks fell lost query demand (content/crosslink lever). Scoped to the surface host via the GSC url_filter. Unmeasurable dimensions return a not_instrumented marker (coverage gap), never a zero.',
	);
}
